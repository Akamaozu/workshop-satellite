...
function take_a_pic(callback) {
  ...
  task.step('select random image from cache', function () {
    if (image_cache.length == 0) {
      ...
      console.log(
        '[warn] QUERYSTRING: ' + process.env.CAMERA_SUBJECT_QUERYSTRING
      );
      ...
      return task.end(
        new Error('no images found for current camera subject settings')
      );
    }

    var image_metadata = image_cache.splice(
      Math.floor(Math.random() * image_cache.length),
      1
    );
  ...
  task.step('fetch image content from url', function () {
    ...
    request(image_metadata.url, { encoding: 'binary' }, function (
      err,
      resp,
      body
    ) {
...
function prime_image_cache(callback) {
  ...
  task.step('verify image cache is empty', function () {
    if (Object.prototype.toString.call(image_cache) !== '[object Array]')
      image_cache = [];
    ...
    request(
      {
        ...
        qs: {
          ...
          keywords:
            process.env.CAMERA_SUBJECT_KEYWORDS || 'mars, moon, jupiter',
          media_type: 'image',
        },
      },
  ...
  task.step('log image data analysis', function () {
    ...
    if (verbose)
      console.log(
        'image metadata cached. total=' +
          images_metadata.length +
          ' keywords=' +
          keywords.join(', ')
      );
